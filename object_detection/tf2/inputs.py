...
def _multiclass_scores_or_one_hot_labels(multiclass_scores,
  ...
  def true_fn():
    return tf.reshape(multiclass_scores,
                      [tf.shape(input=groundtruth_boxes)[0], num_classes])
  def false_fn():
    return tf.one_hot(groundtruth_classes, num_classes)

  return tf.cond(pred=tf.size(input=multiclass_scores) > 0, true_fn=true_fn, false_fn=false_fn)
...
def transform_input_data(tensor_dict,
  ...
  preprocessed_shape = tf.shape(input=preprocessed_resized_image)
  new_height, new_width = preprocessed_shape[1], preprocessed_shape[2]

  im_box = tf.stack([
      0.0, 0.0,
      tf.cast(new_height, dtype=tf.float32) / tf.cast(true_image_shape[0, 0], dtype=tf.float32),
      tf.cast(new_width, dtype=tf.float32) / tf.cast(true_image_shape[0, 1], dtype=tf.float32)
  ])
  ...
  if fields.InputDataFields.groundtruth_boxes in out_tensor_dict:
    out_tensor_dict[fields.InputDataFields.num_groundtruth_boxes] = tf.shape(
        input=out_tensor_dict[fields.InputDataFields.groundtruth_boxes])[0]
