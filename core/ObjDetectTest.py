...
def executeInferenceGraph(pipeline_config_path):
    ...
    if not os.path.exists(pipeline_config_path+'/object_detection_graph'):
        ...
        SMLExportInferenceGraph.main(pipeline_config_path+'/ssd_mobilenet_v1_coco.config', trained_checkpoint, output_directory)
    else:
...
def displayImageWithDetection(image_path):
    ...
    imag_nam = path[len(path) - 1].split('/')
    plt.savefig("static/"+imag_nam[0])
...
def detect(img_location, model_name):
    
    image_name = img_location
    ...
    displayImageWithDetection(image_name)
